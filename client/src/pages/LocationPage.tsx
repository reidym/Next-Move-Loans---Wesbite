...
export default function LocationPage(){
  ...
  return <SiteLayout>
    ...
    <main id="main-content" className="bg-white">
      <section className="py-10 lg:py-14">
        <div className="container grid gap-8 lg:grid-cols-[1fr_0.72fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-3 max-w-4xl text-[clamp(2.3rem,4vw,4rem)] font-black tracking-[-0.045em] text-[#16203A]">Mortgage Broker {location.title}</h1>
          ...
          <div className="rounded-2xl bg-[#EEF7FB] p-6">
            <MapPin className="size-6 text-[#EC7354]"/>
            <h2 className="mt-4 text-xl font-black text-[#16203A]">Why local context matters in {location.title}</h2>
      ...
      <section className="border-y border-[#16203A]/8 py-9 lg:py-11">
        <div className="container">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#EC7354]">POPULAR FINANCE NEEDS</p>
              <h2 className="mt-2 text-2xl font-black text-[#16203A]">What do you need help with in {location.title}?</h2>
            </div>
            <Link className="text-sm font-black text-[#EC7354]" href="/contact-us">Not sure? Ask us →</Link>
          </div>

          <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {priorityServices.map(service=>service?<Link className="group flex min-h-[150px] flex-col justify-between rounded-2xl bg-[#EEF7FB] p-5" href={`/services/${service.slug}`} key={service.slug}>
              <div>
                <h3 className="text-lg font-black text-[#16203A]">{service.title}</h3>
      ...
      {location.localContext?.length ? <section className="py-8 lg:py-10"><div className="container"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">{location.localContext.slice(0,4).map(item=><div className="flex items-start gap-3" key={item}><Check className="mt-1 size-4 shrink-0 text-[#EC7354]"/><p className="text-sm font-semibold leading-6 text-[#16203A]">{item}</p></div>)}</div></div></section>:null}

      <section className="bg-[#F5FAFD] py-9 lg:py-11">
        <div className="container grid gap-7 lg:grid-cols-[0.72fr_1.28fr]">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#EC7354]">COMMON QUESTIONS</p>
            <h2 className="mt-2 text-2xl font-black text-[#16203A]">Working with a mortgage broker in {location.title}</h2>
          </div>
          <FaqBlock items={faqs}/>
        </div>
      </section>

      <section className="bg-[#16203A] py-7 text-white">
        <div className="container flex flex-wrap items-center justify-between gap-4">
          <p className="text-xl font-black">Ready to talk about your next move?</p>
          <div className="flex flex-wrap gap-3"><Link className="button button-coral button-small" href="/contact-us">Contact Us</Link><Link className="button button-outline-light button-small" href="/book-a-call">Book a Call</Link></div>
        </div>
      </section>
    </main>
  </SiteLayout>;
